...
<div class="edit-container">
    ...
    @if(request()->routeIs('company.*'))
        <form method="POST" action="{{ route('company.purchase.update', ['token' => request()->route('token'), 'purchase' => $purchase->id]) }}" class="modern-form">
    @else
        <form method="POST" action="{{ route('admin.company.purchase.update', ['company' => request()->route('company') ?? $purchase->company_id, 'purchase' => $purchase->id]) }}" class="modern-form">
    @endif
        @csrf
        @method('PUT')
            ...
            <div id="receiptBody" class="receipt-body" style="display:none;">
                <div class="receipt-container-viewport">
                    ...
                    <div class="receipt-toolbar-floating">
                        ...
                        <button type="button" id="zoomOut" class="zoom-btn">âˆ’</button>
                    </div>
        ...
        <div class="form-section">
            ...
            <div class="form-actions">
                @if(request()->routeIs('company.*'))
                    <a style="background-color: #ccffcc;" href="{{ route('company.purchase.show', ['token' => request()->route('token'), 'purchase' => $purchase->id]) }}" class="btn-action btn-cancel">
                        Kembali
                    </a>
                @else
                    <a style="background-color: #ccffcc;" href="{{ route('admin.company.purchase.show', ['company' => request()->route('company') ?? $purchase->company_id, 'purchase' => $purchase->id]) }}" class="btn-action btn-cancel">
                        Kembali
                    </a>
                @endif
...
<!-- ==========================================================
   CSS INJECTION KHUSUS HALAMAN EDIT
========================================================== -->
<style>
    /* Scope Wrapper */
    .edit-container {
    ...
    #zoomLevel {
        ...
        color: var(--text-main);
          ...
          /* ==========================================================
       MINIMALIST HORIZONTAL SCROLL FOR MOBILE (DESKTOP STYLE)
    ========================================================== */
    @media (max-width: 768px) {
        /* Memaksa grid Header Invoice tetap 2 kolom menyamping di HP */
        .form-grid {
            grid-template-columns: repeat(2, 1fr); 
            gap: 10px; /* Jarak antar kolom sedikit dirapatkan di HP */
        }

        /* Memaksa tabel tetap horizontal dan bisa digeser ke samping */
        .table-responsive-wrapper {
            width: 100%;
            overflow-x: auto;
        ...
        .edit-table {
            min-width: 480px; /* Batas minimal lebar tabel agar input tidak gepeng */
        }

        .edit-table th, 
        .edit-table td {
            padding: 10px 8px; /* Memperkecil ruang padding di HP */
            font-size: 13px;   /* Ukuran teks sedikit diperkecil */
        }
        ...
        /* Tombol aksi bagian bawah tetap menyesuaikan layar HP */
        .form-actions {
...
<script>
    function toggleReceipt() {
        ...
        if (body.style.display === "none") {
            ...
            arrow.innerHTML = "Ã¢â€“Â² Sembunyikan";
        } else {
            body.style.display = "none";
            arrow.innerHTML = "Ã¢â€“Â¼ Tampilkan";
        }
    }
